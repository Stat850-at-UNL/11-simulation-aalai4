...
![](figure/Craps_simulation_flowchart.png)
...
| **Roll Number** | **Die 1** | **Die 2** | **Total Roll** | **Phase**     | **Outcome** |
|------------|------------|------------|------------|------------|------------|
| 1               | 3         | 4         | 7              | Come-Out Roll | Win         |
| 2               | 2         | 5         | 7              | Point Roll    | Lose        |
| ...             | ...       | ...       | ...            | ...           | ...         |
...
| **Game ID** | **Total Rolls** | **Come-Out Result** | **Point Established** | **Point Value** | **Final Outcome** | **Rolls During Point Phase** |
|-----------|-----------|-----------|-----------|-----------|-----------|-----------|
| 1           | 5               | Lose                | Yes                   | 8               | Win               | 3                            |
| 2           | 3               | Win                 | No                    | N/A             | Win               | 0                            |
| 3           | 7               | Win                 | Yes                   | 5               | Lose              | 5                            |
| ...         | ...             | ...                 | ...                   | ...             | ...               | ...                          |
...
simulate_craps_game <- function() {
  # Input: None
  # Output: A dataframe with columns id, roll, and outcome
  # Description: Simulates a single game of craps by rolling dice and tracking the results of each roll until the game is won or lost.
}
```
...
summarize_craps_game <- function(game_data) {
  # Input: game_data - A data frame returned from simulate_craps_game with columns: id (roll number), roll (total dice roll), phase (Come-Out or Point Roll), outcome (Win, Lose, or Continue)
  # Output: A single-row data frame summarizing the game with columns: n_rolls (number of rolls in the game), outcome (Win or Lose), point (NA if no point was established, otherwise the point value)
  # Description: This function takes the detailed data of a single craps game and summarizes the game into a single row, including the total number of rolls, the final outcome, and the point value if applicable.
```
...
run_craps_simulation <- function(N) {
  # Input: N - An integer specifying the number of craps games to simulate.
  # Output: A data frame summarizing all simulated games, where each row represents a single game with columns:n_rolls (number of rolls in the game), outcome (Win or Lose), point (Point value or NA if no point was established).
  # Description: This function simulates N games of craps using `simulate_craps_game` and summarizes each game using `summarize_craps_game`. It combines the summaries into a single data frame.
}
```
...
I do not know if I am missing anything that would prevent me from writing code implementing craps based on my pseudocode, but I know I'll need to test the functions during the coding process to ensure that they are performing their duties.
If they aren't, I'll work on tweaking them as needed.
Next week, I'll begin writing code based on the pseudocode to simulate the craps game.
